...
keywords:  
summary: 'Le plug-in VMware SnapCenter comprend une base de données MySQL (également appelée base de données NSM) qui contient les métadonnées de toutes les tâches effectuées par le plug-in. Vous devez sauvegarder ce référentiel régulièrement.' 
---
...
[role="lead"]
Le plug-in VMware SnapCenter comprend une base de données MySQL (également appelée base de données NSM) qui contient les métadonnées de toutes les tâches effectuées par le plug-in. Vous devez sauvegarder ce référentiel régulièrement.
