...
[role="lead"]
Le journal d'audit est un ensemble d'événements dans un ordre chronologique, qui est écrit dans un fichier de l'appliance. Les fichiers journaux d'audit sont générés à l' `/var/log/netapp/audit` emplacement et les noms de fichiers suivent l'une des conventions de dénomination suivantes :
